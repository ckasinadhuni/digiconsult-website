...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function getServiceInfo(serviceType) {
        const serviceData = {
            ...
            itom: { name: 'ITOM Operations', description: 'IT Operations Management with advanced analytics' },
            spm: { name: 'SPM Portfolio', description: 'Strategic Portfolio Management excellence' },
            automation: { name: 'Intelligent Automation', description: 'Enterprise automation and AI infrastructure solutions' }
        };
...
function initAuthenticationSystem() {
    ...
    async function checkAuthenticationStatus() {
        try {
            const response = await fetch('https://auth.digiconsult.ca/api/auth/me', {
                method: 'GET',
                credentials: 'include',
                headers: {
            ...
            if (response.ok) {
                ...
                if (data.success && data.data) {
                    updateAuthButtonForLoggedInUser(data.data);
                    return true;
                }
            }
        } catch (error) {
    ...
    function handleAuthButtonClick() {
        const isAuthenticated = authButton.textContent.includes('Dashboard');
        
        if (isAuthenticated) {
            // User is logged in, go to dashboard
            window.location.href = 'https://digiconsult.ca/dashboard';
        } else {
            // User is not logged in, go to auth page
            window.location.href = 'https://auth.digiconsult.ca';
        }
    }
    
    // Update button for logged in user
    function updateAuthButtonForLoggedInUser(userData) {
        authButton.textContent = `Dashboard`;
        authButton.title = `Welcome back, ${userData.firstName}!`;
        authButton.style.background = 'linear-gradient(135deg, #10b981 0%, #059669 100%)';
    }
    ...
    function getAccessToken() {
        ...
        return cookies.access_token || localStorage.getItem('digiconsult_access_token') || '';
    }
    
    // Listen for authentication events from popup/redirect
    window.addEventListener('message', (event) => {
        if (event.origin !== 'https://auth.digiconsult.ca') return;
    ...
    dashboardContainer.innerHTML = `
        ...
        <div class="dashboard-content">
            <div class="services-grid">
                <div class="service-card" data-service="n8n">
                    <h3>Workflow Automation</h3>
                    <p>Access n8n for powerful workflow automation</p>
                    <button class="access-button">Access n8n</button>
                </div>
                
                <div class="service-card" data-service="ollama">
                    <h3>AI Language Models</h3>
                    <p>Chat with advanced AI language models</p>
                    <button class="access-button">Access Ollama</button>
                </div>
                
                <div class="service-card" data-service="whisper">
                    <h3>Speech to Text</h3>
                    <p>Convert audio files to text transcriptions</p>
                    <button class="access-button">Access Whisper</button>
                </div>
                
                <div class="service-card" data-service="ocr">
                    <h3>Document OCR</h3>
                    <p>Extract text from images and documents</p>
                    <button class="access-button">Access OCR</button>
                </div>
            </div>
        </div>
    `;
...
function accessService(service) {
    const serviceUrls = {
        ...
        'whisper': 'https://whisper.digiconsult.ca',
        'ocr': 'https://ocr.digiconsult.ca'
    };
...
async function logout() {
    try {
        await fetch('https://auth.digiconsult.ca/api/auth/logout', {
            method: 'POST',
            credentials: 'include',
            headers: {
                'Authorization': `Bearer ${getAccessToken()}`
    ...
    localStorage.removeItem('digiconsult_access_token');
    window.location.href = 'https://digiconsult.ca';
}
